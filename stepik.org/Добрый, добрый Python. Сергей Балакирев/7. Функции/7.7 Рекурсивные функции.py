...
def rec(word):
    ...
    после каждой буквы поставить скобку. В четной строке центральные 2 буквы
    не должны быть разделены.

    Поэтому делаем проверку на длину центральных букв и возвращаем эти буквы
    Это будет нашим выходом из рекурсии

    Мы знаем, что после первой буквы идет скобка и перед последней тоже,
    поэтому пропишем их сразу, а нашу функцию поместим между ними
    """
